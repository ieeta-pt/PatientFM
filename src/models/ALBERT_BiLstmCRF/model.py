...
class Model:
    def __init__(self, configs, labels_dict, max_length, device):
        ...
        AlBertModelToSize = {"albert-base-v2": 768, "albert-large-v2": 1024, "albert-xlarge-v2": 2048, "albert-xxlarge-v2": 4096}
        self.albert_model = configs.ALBERT_MODEL
        self.albertLayer = AlbertModel.from_pretrained(self.albert_model).to(device=self.device)

        self.USE_NEJI = configs.USE_NEJI
        if self.USE_NEJI == "True":
            self.encoder_insize = AlBertModelToSize[self.albert_model] + 1
        else:
            self.encoder_insize = AlBertModelToSize[self.albert_model]
